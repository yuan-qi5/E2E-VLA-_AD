...
**partial()** 是 Python `functools` 标准库中的函数，用于**冻结**某些函数的参数，返回一个新的函数对象，新函数在调用时，自动带上预先指定的参数，其余参数在后续调用时补齐。

### 45. @staticmethod & @classmethod

**@staticmethod** : 
- 是 Python 内置装饰器，用于定义类中的静态方法。
- 静态方法不需要（也不能）访问类实例（self）与类本身（cls），即方法内部不能用 `self`、`cls`
- 更像 “归属类的普通函数”，逻辑上和类相关，结构上放在类的命名空间里

**@classmethod** :
- 是 python 内置装饰器，用于定义类方法
- 被其修饰的方法，第一个参数为 `cls` 即当前类本身，不是 `self`（实例）
- 类方法可通过类名调用，也可通过实例调用，但接收到的总是类对象而不是实例对象
- 常用于修改类属性、工厂方法等

### 46. `# noqa`

`# noqa` 是 Python 代码中一种注释指令，主要用于告诉静态代码检查工具（如 flake8、pylint 等）忽略本行的警告或错误提示。
